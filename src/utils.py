...
class CSVUrl:
    COVIDPRO_CSV = "https://raw.githubusercontent.com/CEEDS-DEMM/COVID-Pro-Dataset/master/deathsItaProv.csv"  # noqa: E501
    DPC_REGIONI = "https://raw.githubusercontent.com/pcm-dpc/COVID-19/master/dati-regioni/dpc-covid19-ita-regioni.csv"  # noqa: E501
    DPC_PROVINCE = "https://raw.githubusercontent.com/pcm-dpc/COVID-19/master/dati-province/dpc-covid19-ita-province.csv"  # noqa: E501
...
class DataDownloader:
    ...
    def download_covidpro_csv(self):
        self._download(CSVUrl.COVIDPRO_CSV, str(Path(self.path, CSVName.COVIDPRO_CSV)))

    def download_regioni_csv(self):
        self._download(CSVUrl.DPC_REGIONI, str(Path(self.path, CSVName.DPC_REGIONI)))

    def download_province_csv(self):
        self._download(CSVUrl.DPC_PROVINCE, str(Path(self.path, CSVName.DPC_PROVINCE)))
...
def load_csv(path):
    ...
    dpc_province_df.denominazione_provincia = (
        dpc_province_df.denominazione_provincia.str.replace(
    ...
    covidpro_df.Region = covidpro_df.Region.str.replace(
        "P.A. Trento", "Trentino Alto Adige"
    ).replace("P.A. Bolzano", "Trentino Alto Adige")

    covidpro_df.Province = covidpro_df.Province.str.replace(
        "Reggio nell Emilia", "Reggio nell'Emilia"
    ).replace("L Aquila", "L'Aquila")
    pop_prov_df.Territorio = (
        pop_prov_df.Territorio.str.replace("Valle d'Aosta", "Aosta")
...
def fix_trentino(df):
    ...
    res_df = df_trento.drop(columns=cols_to_drop).reset_index(
        drop=True
    ...
    df_filtered = df_filtered.loc[df_filtered.denominazione_regione != "P.A. Trento"]
...
def pre_process_csv(
    ...
    dpc_regioni_df["NC_R_Rolling"] = (
        dpc_regioni_df["NC_R"].rolling(window=window).mean()
    )

    dpc_regioni_df["IC_R_Rolling"] = (
        dpc_regioni_df["IC_R"].rolling(window=window).mean()
    )
